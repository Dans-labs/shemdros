...
public class EnvPool {
	...
	public EmdrosEnv getEmdrosEnv(eOutputKind output_kind, 
			eCharsets charset, 
			String hostname,
			String username,
			String password,
	...
	public EnvWrapper getPooledEnvironment() throws ShemdrosException
	{
		...
		synchronized (pool) {
			...
			if (envWrapper == null)
			{
				EmdrosEnv env = getEmdrosEnv(eOutputKind.swigToEnum(getOutputKind()), 
						eCharsets.swigToEnum(getCharset()), 
						getHostname(), getUsername(), getPassword(), getInitialDB(), 
						eBackendKind.swigToEnum(getBackendKind()));
